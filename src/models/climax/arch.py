...
from scenarIA.src.models.climax.arch_base import ClimaX
from scenarIA.src.models.climax.pos_embed import get_1d_sincos_pos_embed_from_grid
...
class ClimaXClimateBench(ClimaX):
    ...
    def forward_encoder(self, x: torch.Tensor, lead_times: torch.Tensor, variables):
        ...
        print('lead time', lead_times.get_device())
    ...
    def forward(self, x):
        print(x.get_device())
        lead_times = torch.Tensor([0.0]).to(dtype=x.dtype, device=x.device)
        x = x.permute(0, 1, 4, 2, 3).contiguous() # Modified B, T, H, W, C --> B, T, C, H, W 
        x = self.forward_encoder(x, lead_times, self.default_vars)  # B, 1, D
        preds = self.head(x)
        preds = preds.reshape(-1, 1, self.img_size[0], self.img_size[1]) # B, 1, H, W
        return preds

if __name__=='__main__':
    model = ClimaXClimateBench(default_vars=['CO2', 'SO2', 'CH4', 'BC', 'tau'],
        out_vars='pr',
        img_size=[96, 192],
        time_history=5,
        patch_size=16,
        ...
        drop_rate=0.1,
        parallel_patch_embed=False,
        freeze_encoder=False).float()

    summary(model, input_size=(16, 5, 6, 96, 192))
    x = torch.rand((16, 5, 6, 96, 192)).float()
    output = model(x.cuda()).cuda()
    print(output.shape)