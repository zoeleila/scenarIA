...
class ConvGRUCell(nn.Module):
    ...
    def init_hidden(self, batch_size):
        return (Variable(torch.zeros(batch_size, self.hidden_dim, self.height, self.width, device=self.conv_can.weight.device)))
...
class ConvGRU(nn.Module):
    def __init__(self, input_size, input_dim, hidden_dim, kernel_size, num_layers,
        ...
        self.last_conv = nn.Conv2d(self.hidden_dim[-1], 1, kernel_size=1) # Added to return 1 channel output
...
if __name__ == '__main__':
    model = ConvGRU(input_size=(96, 192), input_dim=6, hidden_dim=[64,64], kernel_size=(3, 3),
                     num_layers=2, batch_first=True, bias=True, dtype=None, return_all_layers=False).cuda()
